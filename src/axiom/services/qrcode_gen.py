...
class QRCodeGenerator:
    ...
    def generate_terminal_qr(data: str) -> str:
        ...
        try:
            p = subprocess.Popen(["qrencode", "-t", "ANSIUTF8", data], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def export_png(data: str, output_path: str) -> bool:
        ...
        try:
            subprocess.run(["qrencode", "-o", output_path, data], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            return True
        except Exception:
            return False
