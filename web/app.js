...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tabMeta = {
        overview: { title: "System Overview", subtitle: "Live server telemetry and protocol activity" },
        users: { title: "Users & Access Limits", subtitle: "Manage active accounts and expiration dates" },
        protocols: { title: "Protocols & Tunnels", subtitle: "Generate WireGuard, VLESS Reality, and Hysteria 2 keys" },
        security: { title: "Security & Firewall", subtitle: "Automated vulnerability scanner and nftables control" },
        backup: { title: "Backup & Recovery", subtitle: "Local root-only encrypted disaster recovery archives" }
    };
    ...
    function updateTelemetryUI(data) {
        document.getElementById("val-ram").textContent = `${data.mem_used_mb} MB / ${data.mem_total_mb} MB`;
        document.getElementById("bar-ram").style.width = `${data.mem_percent}%`;

        document.getElementById("val-disk").textContent = `${data.disk_used_gb} GB / ${data.disk_total_gb} GB`;
    ...
    async function loadUsers() {
        ...
        tbody.innerHTML = `
            <tr>
                ...
                <td><button class="btn btn-secondary" style="padding: 0.2rem 0.6rem; font-size: 0.75rem;">Revoke</button></td>
            </tr>
            <tr>
    ...
    document.getElementById("btn-new-user").addEventListener("click", () => modalUser.classList.add("active"));
    document.getElementById("btn-modal-close").addEventListener("click", () => modalUser.classList.remove("active"));
    document.getElementById("btn-modal-cancel").addEventListener("click", () => modalUser.classList.remove("active"));
